...
% this one samples partitions, generating samples based on number of
% state-flips from perceptrons. 
%% set up for 10 cells 
num_cells = 10;
...
for i_group = 1:(num_cells-1)
    ...
    partOut(i_group).group_size = group_size;
    partOut(i_group).cell_inds = cell_inds;
    partOut(i_group).real_cellIDs = real_cellIDs;
    partOut(i_group).setID = i_set;
    ...
    partOut(i_group).input_efficiency = input_efficiency;
    partOut(i_group).inputIpred = inputIpred;
    partOut(i_group).inputEntropy = inputEntropy;
    ...
    wb_i = round(2*rand(10, group_size));
    ...
    %% loop over all the perceptron runs to get a set of starting points
    % first count, for preallocation
    num_Perceptrons = size(X_per.effOut(i_group).init_w.uniqParts, 1);
    ctr = num_Perceptrons;  % initial value of counter
    for i_sam = 1:length(X_per.effOut(i_group).samp_w)
        num_Perceptrons = num_Perceptrons + size(X_per.effOut(i_group).samp_w(i_sam).uniqParts, 1);
    end
    %%
    % then get all perceptron-partitions. These may have some duplicates
    starter_PerceptronPs = zeros(num_Perceptrons, ...
        size(X_per.effOut(i_group).init_w.uniqParts, 2));
    %using that counter again...
    starter_PerceptronPs(1:ctr, :) = X_per.effOut(i_group).init_w.uniqParts;
    for i_sam = 1:length(X_per.effOut(i_group).samp_w)
        nextP = X_per.effOut(i_group).samp_w(i_sam).uniqParts;
        starter_PerceptronPs(ctr + (1:size(nextP, 1)), :) = nextP;
        ctr = ctr + size(nextP, 1);
        
    end
    
    starter_PerceptronPs = returnUniquePartitionSet(starter_PerceptronPs);
    %% compute pred-I over partitions systematically perturbed from known
    % perceptrons
    numBFs = 5; % number of fractions tried for bit-flipping fractions
    fracBitFlips = linspace(0, 0.75, numBFs+1);
    fracBitFlips = fracBitFlips(2:end);
    num_reps = 2;  % number of times to flip bits per input perceptron partition
    tic

    clear bfPartOut
    bfPartOut(numBFs) = struct();
    parfor i_bf = 1:numBFs
        
        % generate set of partitions from perceptron-realized partitions by
        % randomly flipping a fraction fracBitFlips(i_bf) of observed-word
        % partition rules. 
        bf_Partitions = perturbPerceptronPartitions(starter_PerceptronPs, fracBitFlips(i_bf), num_reps);
        
        bf_Partitions = returnUniquePartitionSet(bf_Partitions);
        
        
        [all_Infos, uniqPartitions, uniq_fr, pisUniq, uniq_pI] = computePartitionPI_onManyPartitions(bf_Partitions, group_word_record);
    
        bfPartOut(i_bf).all_Part = bf_Partitions;
        bfPartOut(i_bf).all_info = all_Infos;
        bfPartOut(i_bf).uniqParts = uniqPartitions;
        bfPartOut(i_bf).uniFR = uniq_fr;
        bfPartOut(i_bf).uniPI = uniq_pI;
        bfPartOut(i_bf).pisUniq = pisUniq;
        bfPartOut(i_bf).bitFlipProb = fracBitFlips(i_bf);
        
%         plot(uniq_fr, uniq_pI, '*')
    end
    
    toc
    hold off
    
    partOut(i_group).bfPartOut = bfPartOut;

end

close all
save(['partition_grouprun_maxg' num2str(num_cells) '_set' num2str(i_set)], 'partOut');
end
